...
Value *NodeTern::llvm_codegen(LLVMCompiler *compiler) {
    Value *expression = exp->llvm_codegen(compiler);
    Value *leftptr = left->llvm_codegen(compiler);
    Value *rightptr = right->llvm_codegen(compiler);

    
    return NULL;
}

Value *NodeVAssn::llvm_codegen(LLVMCompiler *compiler) {
    Value *exp = expression->llvm_codegen(compiler);
    AllocaInst *alloc = compiler->locals[identifier];
    return compiler->builder.CreateStore(exp,alloc);
}
...
Value *NodeIfElse::llvm_codegen(LLVMCompiler *compiler) {
    ...
    expression = compiler->builder.CreateICmpNE(expression, ConstantInt::get(Type::getInt64Ty(*(compiler->context)),0),"ifcond");
    Function *func = compiler->builder.GetInsertBlock()->getParent();
    BasicBlock *thenblock = BasicBlock::Create(*compiler->context,"if_true", func);
    BasicBlock *elseblock = BasicBlock::Create(*compiler->context,"if_false");
    BasicBlock *mergeblock = BasicBlock::Create(*compiler->context,"if_merge");

    compiler->builder.CreateCondBr(expression,thenblock,elseblock);
    compiler->builder.SetInsertPoint(thenblock);
    
    Value *expression_if = exp_if->llvm_codegen(compiler);
    if (!expression_if) {
        return nullptr;
    }
    ...
    Value *expression_else = exp_else->llvm_codegen(compiler);
    if (!expression_else) {
        return nullptr;
    }
    compiler->builder.CreateBr(mergeblock);
    ...
    PHINode *pnode = compiler->builder.CreatePHI(compiler->builder.getInt64Ty(), 2, "iftmp");
    pnode->addIncoming(expression_if, thenblock);
    pnode->addIncoming(expression_else, elseblock);
    return pnode;
}
